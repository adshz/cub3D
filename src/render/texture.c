...
void	init_texture_pixels(t_cub *cub)
{
  ...
  if (cub->texture_pixels)
	...
	cub->texture_pixels = ft_calloc(cub->win_height + 1, \
...
void	update_texture_pixels(t_cub *cub, t_texture_data *texture, \
						t_ray *ray, int x)
{
	int	y;
	int	color;
	...
	texture->pos = (ray->draw_start - cub->win_height / 2 + \
	...
	while (y < ray->draw_end)
	{
		...
		color = cub->textures[texture->index][texture->size * \
			texture->y + texture->x];
